...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      numSquaresX.current = Math.ceil(canvas.width / (isHex ? hexHoriz : squareSize)) + 2;
      numSquaresY.current = Math.ceil(canvas.height / (isHex ? hexVert : squareSize)) + 2;
    };

    window.addEventListener('resize', resizeCanvas);
    resizeCanvas();

    const drawHex = (cx: number, cy: number, size: number, ctx: CanvasRenderingContext2D) => {
      ctx.beginPath();
      for (let i = 0; i < 6; i++) {
    ...
    const drawCircle = (cx: number, cy: number, size: number, ctx: CanvasRenderingContext2D) => {
      ctx.beginPath();
      ctx.arc(cx, cy, size / 2, 0, Math.PI * 2);
      ctx.closePath();
    };

    const drawTriangle = (cx: number, cy: number, size: number, flip: boolean, ctx: CanvasRenderingContext2D) => {
      ctx.beginPath();
      if (flip) {
      ...
      if (isHex) {
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            if (alpha) {
              ctx.globalAlpha = alpha;
              drawTriangle(cx, cy, squareSize, flip, ctx);
              ctx.fillStyle = hoverFillColor;
              ctx.fill();
              ctx.globalAlpha = 1;
            }
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            if (alpha) {
              ctx.globalAlpha = alpha;
              drawCircle(cx, cy, squareSize, ctx);
              ctx.fillStyle = hoverFillColor;
              ctx.fill();
              ctx.globalAlpha = 1;
            }
